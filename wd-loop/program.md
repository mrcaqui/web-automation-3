...
5. **SSO 認証の確認**: ブラウザが Workday に認証済みか確認する:
   ```bash
   pnpm exec playwright-cli goto "https://wd5.myworkday.com/cisco/d/home.htmld"
   ```
   URL に `myworkday.com` が含まれれば認証済み。含まれなければループを開始できないため、ユーザーに SSO 認証を依頼する。
6. **プロンプトテンプレートの生成**: シフトデータから種別判定を行い、`.tmp-prompt.txt` を生成する（後述「プロンプトテンプレート」参照）。
7. **results.tsv の初期化**: ヘッダ行のみで作成。ベースラインは最初の実行後に記録する。
8. **確認して開始**: セットアップの確認を取り、実験を開始する。
...
`.tmp-prompt.txt` として保存し、各実験で `claude -p` に渡す。シフトデータから勤務日・Oncall日を計算してテンプレートに埋め込むこと:
