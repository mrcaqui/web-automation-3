...
3. **対象ファイルの読み込み**: 以下を読んで全体像を把握する:
   ...
   - `wd-loop/cleanup-robust.js` — クリーンアップスクリプト（固定、修正不可）。サブフロー C ベース。
...
- `wd-loop/cleanup-robust.js` — クリーンアップロジックは固定。
...
クリーンアップ前に月表示であることを確認する。週表示だとカレンダーイベントの `aria-label` が取得できず削除が空振りする。

```bash
pnpm exec playwright-cli run-code "async page => { const h = page.getByRole('heading', { name: /^\d{4}\u5e74\d{1,2}\u6708$/ }); if (await h.isVisible({ timeout: 3000 }).catch(() => false)) return 'month-view'; await page.locator('[data-automation-id=\"dropdownArrow\"]').first().click(); await page.locator('[data-automation-id=\"dropdown-option\"]', { hasText: '\u6708' }).click(); await h.waitFor({ timeout: 5000 }); return 'switched-to-month-view'; }"
```

### ステップ 3: クリーンアップ（検証リトライループ）

`cleanup-robust.js`（サブフロー C ベース: scrollIntoView, force click, 3500ms wait）を実行し、`verify.js` で残存確認。エントリが残っていれば最大 3 回リトライする。

```bash
for attempt in 1 2 3; do
  echo "=== Cleanup attempt $attempt ==="
  pnpm exec playwright-cli run-code "$(tr '\n' ' ' < wd-loop/cleanup-robust.js)"
  VERIFY=$(pnpm exec playwright-cli run-code "$(tr '\n' ' ' < wd-loop/verify.js)")
  echo "Verify: $VERIFY"
  if echo "$VERIFY" | grep -q '"total":0\|"total": 0'; then
    echo "Cleanup complete"
    break
  fi
  echo "Entries remaining, retrying..."
done
END=$(date +%s)
TIME_SEC=$((END - START))
```

リトライ終了後も `total` が 0 でなければクリーンアップ失敗として記録する（status: `crash`）。
