...
GitHub にも push されるファイル（`src/`, `docs/`, `AGENTS.md`, `package.json` 等）には、社内情報を記載しないこと。
`.gitattributes` で `export-ignore` が設定されているファイル（`.agents/`, `.claude/`, `plans/`, `recordings/` 等）は GitLab のみに保存されるため制約なし。
